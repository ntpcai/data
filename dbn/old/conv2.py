...
def convert_csv_to_json(csv_file, json_file):
        ...
        for row in reader:
            ...
            for k, v in row.items():
                if not should_include(v):
                    continue
                if k in ['strokes'] and v.isdigit():  # Typo in original code? 'strokes' vs 'strokes'
                    processed[k] = int(v)
                else:
                    processed[k] = v
    ...
    # Create hierarchical structure with combined year-set keys
    ...
    for item in sorted(data, key=lambda x: x.get('id', '')):
        ...
        # Add to hierarchy
        hierarchy[publisher][year_set_key][subset].append(entry)
    ...
    def sort_structure(d):
        if isinstance(d, defaultdict):
            sorted_dict = {}
            # Publisher level
            for publisher, year_sets in sorted(d.items(), key=lambda x: x[0]):
                sorted_year_sets = {}
                # Split and sort combined year-set keys
                split_keys = []
                for key in year_sets:
                    year_part, set_part = key.split(' ', 1)
                    split_keys.append((year_part, set_part, key))
                
                # Sort by year then by set
                split_keys.sort(key=lambda x: (get_sort_key(x[0]), get_sort_key(x[1])))
                
                for year_part, set_part, key in split_keys:
                    subsets = year_sets[key]
                    sorted_subsets = sort_subsets(subsets, set_part)
                    sorted_year_sets[key] = dict(sorted_subsets)
                sorted_dict[publisher] = sorted_year_sets
            return sorted_dict
        return d
...
if __name__ == "__main__":
    convert_csv_to_json('data.csv', 'data2.json')
